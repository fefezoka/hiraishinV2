...
namespace Hiraishin.Middleware
{
    public class RequestMiddleware : IMiddleware
    {
        ...
        private async Task HandleNotFoundExceptionAsync(HttpContext context, NotFoundException exception)
        {
            ...
            _logger.LogError(exception, "[{statusCode}] {exception}{stackTrace}", statusCode, exception.Message, exception.StackTrace);
        ...
        private async Task HandleTooManyRequestsExceptionAsync(HttpContext context, TooManyRequestsException exception)
        {
            var contentType = "application/json";
